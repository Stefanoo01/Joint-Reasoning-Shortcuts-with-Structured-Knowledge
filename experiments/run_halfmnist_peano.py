...
def main():
    ...
    bootstrap.add_argument("--preset", type=str, default="peano_medium_v1")
    bootstrap.add_argument("--list_presets", action="store_true")
    bootstrap_args, _ = bootstrap.parse_known_args()

    if bootstrap_args.list_presets:
        print_available_presets("peano")
        return

    preset = get_preset(bootstrap_args.preset)
    if preset.experiment != "peano":
        raise ValueError(f"Preset '{preset.name}' is for '{preset.experiment}', not 'peano'")
    ...
    if selected_preset.experiment != "peano":
        raise ValueError(f"Preset '{selected_preset.name}' is for '{selected_preset.experiment}', not 'peano'")
